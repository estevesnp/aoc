...
pub fn main() !void {
    ...
    for (starts.items) |start| {
        count += try findPaths(matrix.items, start);
    }
...
fn findPaths(matrix: []const []const u8, point: Point) !usize {
    ...
    var finishes = std.AutoHashMap(Point, void).init(arena.allocator());
    defer finishes.deinit();

    try walk(matrix, &finishes, '0', point);
    return finishes.count();
}

fn walk(
    matrix: []const []const u8,
    finishes: *std.AutoHashMap(Point, void),
    char: u8,
    point: Point,
) !void {
    if (char == '9') {
        try finishes.put(point, {});
        return;
    }
